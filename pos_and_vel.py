...
def position(n_dim, empty_space, n_particles_1d, L):
    ...
    if subbox_size_1d < 0:
        raise ValueError("the spawn point is <0 so the particles will not spawn in the correct point")
    ...
    if n_dim==2:
        ...
        ix, iy, iz = np.meshgrid(np.arange(n_particles_1d),np.arange(n_particles_1d),np.arange(n_particles_1d))
